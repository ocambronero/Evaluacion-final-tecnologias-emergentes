    ...
    return (
        <div className="col s12 m4 l3 animated fadeIn fast">
          <div className="card">
            ...
            <div className="card-action"><Link to={`/producto/${this.props.id}`}>Ver más</Link></div>
          </div>
        </div>
    )
  }
...
//--------------------Agregar Productos-----------------------------------------
  agregarProducto(){
     ...
     let cantidadSolicitada = this.state.inputValue
     if (cantidadSolicitada <=0) {
      alert('Seleccione una cantidad válida');
      return
     }
     if(Number(this.state.disponible) < cantidadSolicitada){
       alert('Máxima existencia es: '+ this.state.disponible);
     }else{
       let unidadesDisponibles = (Number(this.state.disponible) - Number(cantidadSolicitada));
       let agregarACarrito = (Number(this.state.contadorCarrito) + Number(cantidadSolicitada));
       this.setState({disponible : unidadesDisponibles});
       this.setState({contadorCarrito : agregarACarrito});
       this.state.productoCarrito.id =  this.props.id;
       this.state.productoCarrito.nombre =  this.props.nombre;
       this.state.productoCarrito.imagen =  this.props.imagen;
       this.state.productoCarrito.precio =  this.props.precio;
       this.state.productoCarrito.disponible =  this.props.disponible;
       this.state.productoCarrito.cantidadSolicitada = (Number(this.state.productoCarrito.cantidadSolicitada) +  Number(cantidadSolicitada));
       this.props.actualizarDisponible(this.state.productoCarrito, cantidadSolicitada, false);
     }
  }
//------------------------------------------------------------------------------
